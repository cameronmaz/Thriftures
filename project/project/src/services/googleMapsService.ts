...
class GoogleMapsService {
  ...
  private userLocation: any | null = null;
  private userLocationMarker: any | null = null;
  private placesService: any | null = null;
  private isApiLoaded = false;
  private loadingPromise: Promise<void> | null = null;
  private markers: any[] = []; // Track markers for cleanup

  async loadGoogleMapsAPI(): Promise<void> {
    // If already loaded, return immediately
    if (this.isApiLoaded && typeof window !== 'undefined' && (window as any).google && (window as any).google.maps) {
  ...
  private addUserLocationMarker(position: { lat: number; lng: number }, isActualLocation: boolean = true): void {
    ...
    this.userLocationMarker = new (window as any).google.maps.Marker({
      position: position,
      map: this.map,
    ...
    try {
      ...
      const searchStrategies = [
        ...
        // Strategy 2: Clothing store search
        {
          location: center,
          radius: radius,
          type: 'clothing_store' as any
        },
        // Strategy 3: Furniture store search
        {
          location: center,
          radius: radius,
          type: 'furniture_store' as any
        },
  ...
  addMarker(place: GooglePlace, onClick?: (place: GooglePlace) => void): any {
    if (!this.map) {
  ...
  isLoaded(): boolean {
    return this.isApiLoaded && typeof window !== 'undefined' && !!(window as any).google && !!(window as any).google.maps;
  }
}
